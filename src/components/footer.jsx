import React from 'react';

export default function Footer() {
    return (
        <footer>
            <div className='footer-cont1'>
                <h3>Convenors</h3>
                <p className='name-footer'>Dr. David Raj Micheal & Dr. Sushmitha P</p>
                <p>Divison of Mathematics, SAS, VIT, Chennai</p>
                <p>Email: davidraj.micheal@vit.ac.in
                     <p>sushmitha.p@vit.ac.in</p>
                    Phone: +91 750287908, +91 9600628354
                </p>
            </div>
            <div className='footer-cont2'>
                <h3>Organizers</h3>
                <p className='name-footer2'>https://vitchennaievents.com</p>
                <p>Vellore Institute of Technology, Chennai</p>
            </div>
            <div className='footer-cont3'>
                <h3>Design and Creation Team</h3>
                <p className='name-footer3'>Shashwat Mishra, Abhinav Kushwaha, Venkat Amith</p>
                <p>github:Shashwatm74,
                    abhii2003<br />
                </p>
            </div>

        </footer>
    )
}